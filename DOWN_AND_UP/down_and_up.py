...
from HELPERS.logger import logger, send_to_logger, send_to_user, send_error_to_user, get_log_channel, log_error_to_channel
...
def _send_video_failure(
    ...
    *,
    ...
    _, text = attach_and_render_terminal_outcome(
        ...
        formatter=format_video_failure_status,
...
def _build_video_terminal_status(user_id: int, delivered_count: int) -> str:
    ...
    return format_video_terminal_status(
...
def _clear_video_subtitle_state(user_id: int, url: str) -> None:
    try:
...
def _finalize_completed_video_outcome(
    *,
    user_id: int,
    proc_msg_id: int,
    message,
    ...
    task_context, success_msg = attach_and_render_terminal_outcome(
        ...
        outcome=outcome,
        task_context=task_context,
        formatter=lambda messages, rendered_outcome: format_video_terminal_status(
            messages,
            rendered_outcome,
            include_credits=not bool(getattr(Config, "HIDE_CREDITS_MSG", False)),
            credits_msg=credits_msg,
...
def _finalize_split_video_success_after_quality_key_error(
    *,
    user_id: int,
    proc_msg_id: int,
    message,
...
def _cleanup_successful_video_download_dir(user_id: int) -> None:
    try:
...
def _send_playlist_video_terminal_status(
    *,
    app,
    user_id: int,
    ...
    send_playlist_cache_status(
        ...
        text=safe_get_messages(user_id).PLAYLIST_VIDEOS_SENT_MSG.format(
...
@dataclass(frozen=True)
class SplitQualityKeyTerminalPlan:
    mode: str
    should_finalize: bool
    should_cache: bool
...
def _build_split_quality_key_terminal_plan(
    *,
    split_msg_ids: list,
    is_playlist: bool,
    video_count: int,
    ...
    return SplitQualityKeyTerminalPlan(
        mode=recovery_plan.mode,
        should_finalize=recovery_plan.should_finalize,
        should_cache=recovery_plan.should_cache,
        final_video_count=len(split_msg_ids) if split_msg_ids and not is_playlist else video_count,
    )
...
def _is_paid_video_message(video_msg) -> bool:
    try:
        return (
...
def _send_open_nsfw_history_copy(
    *,
    message,
    video_path: str,
    caption_text: str,
    ...
    if not log_channel_nsfw:
        logger.warning(
            f"down_and_up: NSFW channel not available (ID: {log_channel_nsfw}), "
            "skipping open copy"
        )
        return False
    try:
        ...
        logger.info("down_and_up: NSFW content open copy sent to NSFW channel for history")
        return True
    except Exception as e:
...
def _route_uploaded_video_to_logs(
    *,
    message,
    user_id: int,
    video_msg,
...
def _cache_playlist_video_delivery(
    *,
    ...
    cached_check = get_cached_playlist_videos(
        get_clean_playlist_url(url), safe_quality_key, [current_video_index]
    )
    logger.info(f"Checking the cache immediately after writing: {cached_check}")
    playlist_indices.append(current_video_index)
    playlist_msg_ids.extend([m.id for m in forwarded_msgs])
...
def _cache_single_video_delivery(
    *,
    forwarded_msgs: list,
    url: str,
    safe_quality_key: str,
    ...
    if not is_nsfw and not need_subs:
        ...
        logger.info(f"NSFW content not cached{log_suffix}")
    elif need_subs:
        logger.info(
            f"Video with subtitles is not cached{log_suffix} - "
            "different users may need different languages"
        )
...
def _handle_quality_key_error(e: Exception, split_msg_ids: list, is_playlist: bool, successful_uploads: int, indices_to_download: list, video_count: int, user_id: int, proc_msg_id: int, message, app, url: str = None, safe_quality_key: str = None):
    ...
    recovery_plan = _build_split_quality_key_terminal_plan(
        split_msg_ids=split_msg_ids,
        is_playlist=is_playlist,
        video_count=video_count,
        url=url,
...
def _resolve_video_task_inputs(
    *,
    task_context: RuntimeTask | None,
    user_id: int,
    message,
    ...
    return (
        ...
        task_context.tags_text,
        bool(task_context.force_no_title or force_no_title),
...
def _resolve_initial_video_subtitles(user_id: int, url: str, quality_key: str):
    subs_enabled = is_subs_enabled(user_id)
    found_type = None
    need_subs = False
    is_always_ask_mode = is_subs_always_ask(user_id)
    ...
    if is_always_ask_mode and is_youtube_url(url):
        subs_lang = get_user_subs_language(user_id)
        need_subs = bool(subs_lang and subs_lang not in ["OFF"])
        logger.info(
            "Always Ask mode: %s -> %s",
            f"user selected language '{subs_lang}'" if need_subs else "no language selected",
            "FORCE SUBS" if need_subs else "NO SUBS",
        )
    elif subs_enabled and is_youtube_url(url):
        found_type = check_subs_availability(
            url,
            user_id,
            quality_key,
            return_type=True,
        ...
        need_subs = determine_need_subs(subs_enabled, found_type, user_id)
        logger.info(
            "Manual mode: subs_enabled=%s, found_type=%s, need_subs=%s, user_id=%s",
            subs_enabled,
            found_type,
            need_subs,
            user_id,
        ...
        logger.info(
            "Subtitle check skipped: subs_enabled=%s, is_youtube=%s, user_id=%s",
            subs_enabled,
            is_youtube_url(url),
            user_id,
        )
    ...
    if is_youtube_url(url):
        subs_lang = get_user_subs_language(user_id)
        logger.info(
            "Final debug: is_always_ask_mode=%s, subs_lang='%s', need_subs=%s, user_id=%s",
            is_always_ask_mode,
            subs_lang,
            need_subs,
            user_id,
    ...
    return subs_enabled, found_type, need_subs


def _try_remote_youtube_cookie_sources(
    *,
    user_id: int,
    url: str,
...
def _resolve_youtube_cookiefile(
    *,
    ...
    if user_cookie_exists:
        ...
        if cookie_urls:
            resolved_cookie = _try_remote_youtube_cookie_sources(
                user_id=user_id,
                url=url,
...
def _resolve_video_cookiefile(*, user_id: int, url: str, user_dir_name: str):
    download_cookie_path = os.path.join(user_dir_name, "cookie.txt")
    user_cookie_path = os.path.join("users", str(user_id), "cookie.txt")
    downloaded_cookie_path = os.path.join(user_dir_name, "_downloaded_cookie.txt")

    if is_no_cookie_domain(url):
        logger.info("Using --no-cookies for domain: %s", url)
        return None

    if is_youtube_url(url):
        return _resolve_youtube_cookiefile(
            user_id=user_id,
            url=url,
    ...
    if cache_result and cache_result["result"]:
        logger.info("Using cached cookies for non-YouTube URL: %s", url)
        return cache_result["cookie_path"]

    if os.path.exists(download_cookie_path):
        logger.info("Using cookies from download directory: %s", download_cookie_path)
        return download_cookie_path

    if os.path.exists(user_cookie_path):
        logger.info("Using user cookies for non-YouTube URL: %s", url)
        return user_cookie_path

    logger.info(
        "No user cookies found for non-YouTube URL: %s, will try fallback during download",
        url,
    )
    return None


def _get_video_format_check_info(
    *,
    url: str,
    user_id: int,
    cached_video_info,
...
def _render_available_video_formats(available_formats):
    video_formats = [
        fmt
...
def _ensure_requested_video_format_available(
    *,
    ...
    if not requested_format or requested_format == "best":
        return True
    ...
    if requested_format.startswith("id:"):
        ...
        logger.warning("Format ID %s not found for this video", requested_id)
        try:
            available_ids = [fmt.get("format_id", "unknown") for fmt in available_formats[:10]]
            logger.info("Available format IDs: %s", available_ids)
            _send_video_failure(
                message,
                user_id,
            ...
            logger.error("Error sending format not found message: %s", e)
        return False

    if "av01" not in requested_format:
        return True
    ...
    logger.warning("AV1 format requested but not available for this video")
    formats_text = _render_available_video_formats(available_formats)
    try:
        safe_edit_message_text(
            user_id,
            proc_msg_id,
            f"{current_total_process}\n"
            f"{safe_get_messages(user_id).DOWN_UP_AV1_NOT_AVAILABLE_MSG.format(formats_text=formats_text)}",
        )
    except Exception as e:
        logger.error("Failed to notify user about format unavailability: %s", e)
...
def _normalize_extracted_video_info(info_dict, *, current_index: int, use_range_download: bool):
    ...
    if isinstance(info_dict, dict) and "entries" in info_dict:
        ...
        if len(entries) > 1:
            ...
            raise Exception(
                f"Video index {current_index} out of range (total {len(entries)})"
            )
        return entries[0]
...
def _apply_video_proxy_settings(ytdl_opts, *, use_proxy: bool, url: str, user_id: int):
    if use_proxy:
        ...
        if proxy_config and "type" in proxy_config and "ip" in proxy_config and "port" in proxy_config:
            ...
            logger.info("Force using proxy for download: %s", proxy_url)
            return ytdl_opts
...
def _handle_live_stream_info(
    *,
    ...
    if not (info_dict and isinstance(info_dict, dict) and info_dict.get("is_live", False)):
        return None
    if LimitsConfig.ENABLE_LIVE_STREAM_BLOCKING:
        return None

    logger.info(
        "Live stream detected but detection is disabled, using live stream downloader for user %s: %s",
        user_id,
        url,
    )
    ...
    return info_dict if result else False


def _configure_video_hls_mode(ytdl_opts, *, url: str, info_dict):
    requested_formats = []
    try:
    ...
    hls_in_requested = any(
        "m3u8" in (rf.get("protocol") or "").lower()
        or "hls" in (rf.get("protocol") or "").lower()
        for rf in requested_formats
    )
    is_hls = (
        "m3u8" in url.lower()
        or (info_dict and info_dict.get("protocol") in ("m3u8", "m3u8_native"))
        or hls_in_requested
    )
    if is_hls:
...
def _update_video_download_mode_status(
    *,
    user_id: int,
    proc_msg_id: int,
    current_total_process: str,
    ...
    try:
        if is_hls:
            ...
            safe_edit_message_text(
                ...
                proc_msg_id,
                f"{current_total_process}\n> <i>"
                f"{safe_get_messages(user_id).ALWAYS_ASK_DOWNLOADING_FORMAT_USING_MSG} "
                f"{ytdl_opts.get('format', 'default')}...</i>",
            )
    except Exception as e:
        logger.error("Status update error: %s", e)
        if "'quality_key'" in str(e):
            _handle_quality_key_error(
                e,
                split_msg_ids,
                is_playlist,
                successful_uploads,
...
def _build_video_error_summary(user_id: int, error_message: str):
    ...
    if "HTTP Error 403" in error_message:
        ...
        if "code 1" in error_message:
            error_description = (
                "FFmpeg processing failed - video format may be incompatible or corrupted"
            )
...
def _maybe_auto_rotate_ip_for_sign_in_required(user_id: int, error_message: str):
    if "Sign in to confirm" not in error_message:
        return
    try:
...
def _build_gallery_dl_fallback_text(
    *,
    ...
    logger.info("[FALLBACK DEBUG] Using saved original_message_text: %s", original_message_text)
    ...
    if range_url_match:
        ...
        logger.info(
            "[FALLBACK DEBUG] FOUND RANGE: %s with range %s-%s",
            parsed_url,
            start_range,
            end_range,
        )
        ...
        start_range = 1
        end_range = 1
        logger.info("[FALLBACK DEBUG] NO RANGE FOUND, using url: %s", parsed_url)

    current_item_index = current_index + 1
    fallback_text = f"/img {current_item_index}-{current_item_index} {parsed_url}"
    logger.info(
        "[FALLBACK] Downloading only failed item %s via gallery-dl, fallback_text: %s",
        current_item_index,
        fallback_text,
    )
    ...
    return fallback_text, start_range, end_range


def _try_gallery_dl_fallback(
    *,
    app,
    message,
    user_id: int,
    ...
    try:
        ...
        logger.error("Failed to import gallery-dl fallback handlers%s: %s", log_suffix, imp_e)
        return None
    ...
    try:
        ...
        fallback_text, start_range, end_range = _build_gallery_dl_fallback_text(
        ...
        if is_nsfw:
            logger.info("[FALLBACK] Added #nsfw tag for NSFW content: %s", url)
        ...
        logger.info(
            "Triggered gallery-dl fallback via /img%s, is_nsfw=%s, range=%s-%s",
            log_suffix,
            is_nsfw,
            start_range,
            end_range,
    ...
    *,
    ...
    if is_youtube_url(url):
        retry_result, did_proxy_retry = maybe_retry_with_proxy_on_geo_error(
            ...
            download_fn=try_download,
            download_args=(url, attempt_opts),
            logger=logger,
            success_log_text="Download retry with proxy successful for user {user_id}",
            failure_log_text="Download retry with proxy failed for user {user_id}",
        )
        return retry_result, did_proxy_retry

    logger.info(
        "Non-YouTube download error detected for user %s, attempting cookie fallback",
        user_id,
    )
    ...
    if retry_result is not None:
        logger.info("Download retry with cookie fallback successful for user %s", user_id)
    else:
        logger.warning("Download retry with cookie fallback failed for user %s", user_id)
    return retry_result, did_proxy_retry


def _render_final_video_download_error(user_id: int, error_message: str):
    error_code, error_description = _build_video_error_summary(user_id, error_message)
    _maybe_auto_rotate_ip_for_sign_in_required(user_id, error_message)
    return (
...
def _handle_generic_video_download_exception(
    *,
    app,
    message,
    user_id: int,
    ...
    if "No videos found in playlist" in error_text or "Unsupported URL" in error_text:
        fallback_result = _try_gallery_dl_fallback(
            app=app,
            message=message,
            user_id=user_id,
            url=url,
    ...
    if "No videos found in playlist" in error_text:
        rendered_text = safe_get_messages(user_id).DOWN_UP_NO_VIDEOS_PLAYLIST_MSG.format(
        ...
        logger.info("Stopping download: playlist item at index %s (no video found)", current_index)
        return "STOP"

    if "TikTok API keeps sending the same page" in error_text and "infinite loop" in error_text:
        rendered_text = safe_get_messages(user_id).VIDEO_TIKTOK_API_ERROR_SKIP_MSG.format(
            index=current_index + 1
        )
        _send_video_failure(
            message,
        ...
        logger.info("Skipping TikTok video at index %s due to API error", current_index)
        return "SKIP"
    ...
    _send_video_failure(
        ...
        failure_kind="download_failed",
        error_text=error_text,
        rendered_text=safe_get_messages(user_id).UNKNOWN_ERROR_MSG.format(error=error),
        task_context=task_context,
    )
    return None
...
def down_and_up(app, message, url=None, playlist_name=None, video_count=1, video_start_with=1, tags_text="", force_no_title=False, format_override=None, quality_key=None, cookies_already_checked=False, use_proxy=False, cached_video_info=None, clear_subs_cache_on_start=True, task_context: RuntimeTask | None = None):
    ...
    if format_override and '/bestaudio' in format_override:
        ...
        audio_branch_result = redirected_audio_branch(
            ...
            quality_key=quality_key,
            format_override=format_override,
            reason="format_override_requests_bestaudio",
        )
    ...
    subs_enabled, found_type, need_subs = _resolve_initial_video_subtitles(
        user_id,
        url,
        safe_quality_key,
    )
    ...
    if safe_quality_key and is_playlist:
        ...
        if cached_videos and (not use_range_download or len(uncached_indices) == 0):
            ...
            if len(uncached_indices) == 0:
                send_playlist_cache_status(
                    ...
                    text=safe_get_messages(user_id).PLAYLIST_SENT_FROM_CACHE_MSG.format(
        ...
        if not need_subs and not is_subs_always_ask(user_id):
            ...
            cached_ids, is_nsfw = lookup_single_cached_ids(
                user_id=user_id,
                url=url,
                quality_key=safe_quality_key,
                user_forced_nsfw=user_forced_nsfw,
                always_ask_enabled=False,
                is_nsfw_detector=is_porn,
                get_cached_message_ids=get_cached_message_ids,
                logger=logger,
                nsfw_skip_log="[VIDEO CACHE] Skipping cache check for NSFW content: url={url}, quality={quality}",
                always_ask_skip_log="[VIDEO CACHE] Skipping cache check because Always Ask mode is enabled: url={url}, quality={quality}",
            )
            if cached_ids:
            ...
            if cached_ids:
                ...
                if not send_as_file:
                    replayed = try_repost_single_cached_media(
                        ...
                        resolve_from_chat_id=lambda: get_log_channel("video"),
                        success_text=safe_get_messages(user_id).VIDEO_SENT_FROM_CACHE_MSG,
                        success_log_text=LoggerMsg.VIDEO_SENT_FROM_CACHE.format(quality=safe_quality_key, user_id=user_id),
                        logger=logger,
                        replay_log_prefix="[VIDEO CACHE] Reposting video",
                        replay_error_prefix="Error reposting video from cache",
                        on_replay_error=lambda: _save_video_cache_with_logging(url, safe_quality_key, [], original_text="", user_id=user_id),
    ...
    try:
        ...
        user_dir, user_dir_name = ensure_user_download_dir(
            user_id=user_id,
            url=url,
            logger=logger,
        )
        cleanup_download_dir_before_start(
            download_dir=user_dir_name,
            message=message,
            logger=logger,
        )
        ...
        def progress_func(d):
            ...
            if d.get("status") == "downloading":
                ...
                _send_video_failure(
                    ...
                    user_id,
                    failure_kind="download_failed",
                    rendered_text=safe_get_messages(user_id).DOWNLOAD_ERROR_GENERIC,
                    use_error_channel=True,
                    task_context=task_context,
                )
        ...
        def try_download(url, attempt_opts):
            ...
            common_opts["cookiefile"] = _resolve_video_cookiefile(
                user_id=user_id,
                url=url,
            ...
            ytdl_opts = _apply_video_proxy_settings(
                ytdl_opts,
                use_proxy=use_proxy,
                url=url,
                user_id=user_id,
            ...
            try:
                ...
                check_info = _get_video_format_check_info(
                    url=url,
                    user_id=user_id,
                    cached_video_info=cached_video_info,
                    cookies_already_checked=cookies_already_checked,
                ...
                if not _ensure_requested_video_format_available(
                ...
                info_dict = _normalize_extracted_video_info(
                    info_dict,
                    current_index=current_index,
                    use_range_download=use_range_download,
                )

                live_result = _handle_live_stream_info(
                    info_dict=info_dict,
                    user_id=user_id,
                    url=url,
                    app=app,
                ...
                is_hls = _configure_video_hls_mode(
                    ytdl_opts,
                    url=url,
                    info_dict=info_dict,
                )
                _update_video_download_mode_status(
                    user_id=user_id,
                    proc_msg_id=proc_msg_id,
                    current_total_process=current_total_process,
                    ytdl_opts=ytdl_opts,
                ...
                result = _run_video_download_phase(
                    ytdl_opts=ytdl_opts,
                    url=url,
                    user_id=user_id,
                    proc_msg_id=proc_msg_id,
                ...
                retry_result, did_proxy_retry = _maybe_retry_video_download_after_error(
                    user_id=user_id,
                    url=url,
                    error_message=error_message,
                    did_proxy_retry=did_proxy_retry,
                    try_download=try_download,
                    attempt_opts=attempt_opts,
                )
                if retry_result is not None:
                    return retry_result
                ...
                if not error_message_sent:
                    _send_video_failure(
                        ...
                        failure_kind="download_failed",
                        error_text=error_message,
                        rendered_text=_render_final_video_download_error(
                            user_id,
                            error_message,
                        ),
                        use_error_channel=True,
                ...
                return _handle_generic_video_download_exception(
                    app=app,
                    message=message,
                    user_id=user_id,
                    url=url,
                ...
                if error_message and not error_message_sent:
                    ...
                    if "Postprocessing" in error_message and "Invalid argument" in error_message:
                        ...
                        _send_video_failure(
                            ...
                            user_id,
                            failure_kind="format_unavailable",
                            rendered_text=format_error_message,
                            use_error_channel=True,
                            task_context=task_context,
                        )
                        error_message_sent = True
...
"""
            ...
            if not downloaded_file:
                ...
                if not files:
                    _send_video_failure(
                        ...
                        rendered_text=safe_get_messages(user_id).SKIPPING_UNSUPPORTED_FILE_TYPE_MSG.format(index=current_index),
                        use_error_channel=True,
                        task_context=task_context,
                    )
                    continue
            ...
            if final_name.lower().endswith((".webm", ".ts")):
                ...
                if not ffmpeg_path:
                    _send_video_failure(
                        ...
                        rendered_text=safe_get_messages(user_id).FFMPEG_NOT_FOUND_MSG,
                        use_error_channel=True,
                        task_context=task_context,
                    )
                    break
                ...
                try:
                    ...
                    _send_video_failure(
                        ...
                        failure_kind="conversion_failed",
                        error_text=str(e),
                        rendered_text=safe_get_messages(user_id).CONVERSION_TO_MP4_FAILED_MSG.format(error=e),
                        use_error_channel=True,
                        task_context=task_context,
                    )
                    break
            ...
            if int(video_size_in_bytes) > max_size:
                ...
                for p in range(len(caption_lst) if caption_lst else 0):
                    ...
                    try:
                        route_result = _route_uploaded_video_to_logs(
                            message=message,
                            user_id=user_id,
                            video_msg=video_msg,
                            url=url,
                            ...
                            is_playlist=is_playlist,
                            is_split_item=bool(caption_lst and len(caption_lst) > 1),
                            video_path=path_lst[p],
                            caption_text=caption_lst[p] if caption_lst and p < len(caption_lst) else f"part_{p+1}",
                            duration=part_duration,
                            width=width,
                            height=height,
                            thumb_path=splited_thumb_dir,
                        )
                        forwarded_msgs = route_result["forwarded_msgs"]
                        already_forwarded_to_log = route_result["already_forwarded_to_log"]
                        is_nsfw = route_result["is_nsfw"]
                ...
                if final_name:
                    ...
                    try:
                        ...
                        _send_video_failure(
                            ...
                            error_text=str(e),
                            rendered_text=safe_get_messages(user_id).ERROR_SENDING_VIDEO_MSG.format(error=str(e)),
